...
{ _LOGDEBUG(insAddress << " JMP_BD_16 base " << baseReg << " depl " << displ << " saut" << ea); }
...
{ _LOGDEBUG(insAddress << " JMP_ISD_16 index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }
...
{ _LOGDEBUG(insAddress << " JMP_BISD_16 base " << baseReg << " index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }


void UNCONDITIONAL_BR::sJMP_BD_32(THREADID tid, ADDRINT ea, REG baseReg, ADDRINT baseRegValue, INT32 displ ADDRESS_DEBUG)
{ _LOGDEBUG(insAddress << " JMP_BD_32 base " << baseReg << " depl " << displ << " saut" << ea); }

void UNCONDITIONAL_BR::sJMP_ISD_32(THREADID tid, ADDRINT ea, REG indexReg, ADDRINT indexRegValue, 
                 UINT32 scale, INT32 displ ADDRESS_DEBUG)
{ _LOGDEBUG(insAddress << " JMP_ISD_32 index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }

void UNCONDITIONAL_BR::sJMP_BISD_32(THREADID tid, ADDRINT ea, REG baseReg, ADDRINT baseRegValue, 
                  REG indexReg, ADDRINT indexRegValue, UINT32 scale, INT32 displ ADDRESS_DEBUG)
{ _LOGDEBUG(insAddress << " JMP_BISD_32 base " << baseReg << " index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }
...
{ _LOGDEBUG(insAddress << " JMP_BD_64 base " << baseReg << " depl " << displ << " saut" << ea); }

void UNCONDITIONAL_BR::sJMP_ISD_64(THREADID tid, ADDRINT ea, REG indexReg, ADDRINT indexRegValue, 
                 UINT32 scale, INT32 displ ADDRESS_DEBUG)
{ _LOGDEBUG(insAddress << " JMP_ISD_64 index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }

void UNCONDITIONAL_BR::sJMP_BISD_64(THREADID tid, ADDRINT ea, REG baseReg, ADDRINT baseRegValue, 
                  REG indexReg, ADDRINT indexRegValue, UINT32 scale, INT32 displ ADDRESS_DEBUG)
{ _LOGDEBUG(insAddress << " JMP_BISD_64 base " << baseReg << " index " << indexReg << " scale " << scale << " displ " << displ << "saut" << ea); }
